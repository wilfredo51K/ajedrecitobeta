...
//configurar la onda del confetti 
var termino=false;
const iniciarconfe = () => {
    setTimeout(function() {
        confetti.start()
...
//la barra de progresoooooooooooooooooooooooo
var lemonCount = 0;
var detener=false;
...
var demo = lemonCount;
if(detener==false){
setInterval(function(){ updateProgress(demo), demo = (demo >= 100 ? 56.8 : demo + 16.7) }, 700);
}
...
//configuracion para los cuadritos
const FILES=8;
const RANKS=8;
...
play.style.left=pizaleft+252+"px";
play.style.top=pizatop-380+"px";
...
//desombrear filas y columnassssssssssssssssssss
function desombrearfila(fila,columna){
    hayalgosombreado=false;
    for(let y=0; y<RANKS; y++){
        if(y!=columna){
            var a1=posi[fila][y].x1; 
            var a2=posi[fila][y].y1; 
            let rectcolor;
           
                rectcolor=theme.light;
                if((fila+y)%2){ //condicional para el color de los cuadritos
                    rectcolor=theme.dark;  
                }   
                ctx.globalAlpha=1;
             ctx.fillStyle=rectcolor;
...
function desombrearcol(fila,columna){
    for(let y=0; y<FILES; y++){
        if(y!=fila){
            var a1=posi[y][columna].x1; 
            var a2=posi[y][columna].y1; 
            let rectcolor;
            ctx.globalAlpha=1;
                
                rectcolor=theme.light;
                if((columna+y)%2){ //condicional para el color de los cuadritos
                    rectcolor=theme.dark;  
                }   
...
//desombrear diagonalesssssssssssssss
function desombreardiagonal1(fila,columna){
    hayalgosombreado=false;
    var y=columna; 
    var x=fila;
    while(y<RANKS && x<RANKS){
        if(y!=columna){
            var a1=posi[x][y].x1; 
            var a2=posi[x][y].y1; 
            let rectcolor;
...
function desombreardiagonal2(fila,columna){
    var y=fila; 
    var x=columna;
    while(y<FILES && x>=0){
        if(y!=fila){
...
function selecciono(e){
    ...
    //elimino el evento de los botones
    for(var i=0; i<RANKS; i++){ //eliminar para la columna
        if(i!=columnap){
            var elide=filap+'b'+i;
            const but=document.getElementById('a'+ elide);
    ...
    for(var i=0; i<RANKS; i++){ //eliminar para la fila
        if(i!=filap){
            var elide=i+'b'+columnap;
            const but=document.getElementById('a'+ elide);
    ...
    desombrearcol(filap, columnap);
    desombreardiagonal1(filap, columnap);
    desombreardiagonal2(filap, columnap);
    ...
    if(filap==filastar && columnap==columnastar){
        ...
        //abrir el enlace
        setTimeout(function() {location.href = "e5.html"}, 8000);
...
//dibujar columnas 
function dibujarcolumnas(sombra, fila, columna){
    for(let y=0; y<FILES; y++){
        if(y==fila+1 || y==fila-1){
            var a1=posi[y][columna].x1; 
            var a2=posi[y][columna].y1; 
            let rectcolor;

            var elide=y+'b'+columna;
            const but=document.getElementById('a'+ elide);
...
//diagonalessssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
//dibujar diagonal 1
function dibujardiagonal1(sombra, fila, columna){
    hayalgosombreado=true;
    var y=columna; 
    var x=fila;
    var conta=0;
    while(y<RANKS && x<RANKS && conta<2){
        conta++;
        if(y!=columna){
            var a1=posi[x][y].x1; 
            var a2=posi[x][y].y1; 
...
//dibujar diagonal 2
function dibujardiagonal2(sombra, fila, columna){
    var y=fila; 
    var x=columna;
    var conta=0;
    while(y<FILES && x>=0 && conta<2){
        conta++;
        if(y!=fila){
            var a1=posi[y][x].x1; 
            var a2=posi[y][x].y1; 
...
columnastar=getRandomInt(0,8);
if(columnastar==columnap){
     columnastar++;
    if(columnastar>7){
        columnastar-=2;
    }
...
function sombrear(condi,fila, columna){
    ...
    //sombrar de arriba a abajo
    dibujarfilas(condi, fila, columna);
    dibujardiagonal1(condi, fila, columna);

    //sombrar a los lados 
    dibujarcolumnas(condi, fila, columna);
    dibujardiagonal2(condi, fila, columna);
